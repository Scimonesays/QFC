...
class QuartetPaymentCalculatorApp(tk.Tk):
    ...
    def _build_payments_tab(self, main: ttk.Frame) -> None:
        ...
        ttk.Button(main, text="Save & Log Payment", command=self.on_save_and_log_payment).grid(
            row=r, column=0, sticky=tk.W, **pad
        )
        ttk.Button(main, text="Export Spreadsheet", command=self.on_export_spreadsheet).grid(
            row=r, column=1, sticky=tk.W, **pad
        )
        ttk.Button(main, text="Reset", command=self.on_reset).grid(
        ...
        self._lbl_save_feedback.grid(
            row=r, column=0, columnspan=3, sticky=tk.W, padx=5, pady=(2, 0)
        )
        r += 1
        ...
        tk.Label(
            ...
            fg="#666666",
        ).grid(row=r, column=0, columnspan=3, sticky=tk.W, padx=5, pady=(12, 0))
